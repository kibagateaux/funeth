...
contract FunCityLandRegistry is ERC721, GPv2Helper {
    ...
    IFunETH public immutable funETH;
    IFunETH public immutable nnUSDC;
    IERC20x public immutable USDC;
    IERC20x public immutable WETH;
    ...
    constructor(address _funETH, address _nnUSDC, address _curator) ERC721() {
        funETH = IFunETH(_funETH);
        nnUSDC = IFunETH(_nnUSDC);
    ...
    //
    function initialize(address _funETH, address _nnUSDC) public {
    ...
    function stake(uint64 lotID, address owner, address nnToken, uint128 stakeAmount) public returns (uint256) {
        ...
        if (nnToken != address(funETH) && nnToken != address(nnUSDC)) {
    ...
    function depositETHAndStake(uint64 lotID, address owner) public payable returns (uint256) {
        ...
        WETH.approve(address(funETH), uint128(msg.value));
        funETH.depositWithPreference(uint128(msg.value), _properties[lotID].city, address(this));
    ...
    function initiateOrder(Stake memory partron, uint64 lotID) internal virtual returns (GPv2Order.Data memory) {
        ...
        // emit OrderInitiated(address(funETH), address(nnUSDC), order.hash(GPv2Order.COWSWAP_DOMAIN_SEPARATOR), partron.stakeAmount, _properties[lotID].price, deadline);
