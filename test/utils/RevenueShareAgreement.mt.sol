...
// import { RevenueShareAgreement } from "../../src/utils/RevenueShareAgreement.sol";
// import {Ownable} from "solady/auth/Ownable.sol";
// import { MockFeeGenerator } from "../helpers/MockFeeGenerator.sol";

// import {GPv2Order} from "../../src/lib/GPv2.sol";
// import { IFeeClaimer, IRevenueShareAgreement } from "../../src/Interfaces.sol";
...
//     MockToken private revenueToken;
//     MockToken private creditToken;
//     uint256 private lenderRevenueSplit = 47;
...
//         revenueToken = new MockToken();
//         creditToken = new MockToken();
...
//         hoax(borrower);
//         rsa.transferOwnership(address(rsa));
...
//     **********************
    ...
    //     assertEq(initialPrincipal, rsa.totalOwed());
    ...
    // //         vm.expectRevert(RevenueShareAgreement.InvalidRevenueSplit.selector);
    // //     }
    // //     _initRSA(
    // //         address(creditToken),
    // //         initialPrincipal,
    ...
    //         vm.expectRevert(RevenueShareAgreement.InvalidPaymentSetting.selector);
    //     }
    //     _initRSA(
    //         address(creditToken),
    //         _principal,
    ...
    // //     vm.expectRevert(RevenueShareAgreement.InvalidSpigotAddress.selector);
    // //     factory.deployRevenueShareAgreement(
    // //         borrower,
    // //         address(0),  // here
    // //         address(creditToken),
...
//     // any tets for Proxy that we need to check e.g. same byte code for all deployed contracts?]
    ...
    //     deal(address(creditToken), lender, rsa.totalOwed());
    ...
    //     assertEq(lenderBalance1, lenderBalance0 + rsa.totalOwed());
    ...
    //     assertEq(lenderBalance2, lenderBalance1 - rsa.totalOwed(), 'bad post deposit() lender balance');
    //     // ensure proper amount moved to borrower
    //     assertEq(borrowerBalance1, borrowerBalance0 + rsa.totalOwed(), 'bad post deposit() borrower balance');
    ...
    //     vm.startPrank(lender);
    //     vm.expectRevert(RevenueShareAgreement.DepositsFull.selector);
    //     // reverts before MockToken transfer so no approval needed
    //     rsa.deposit(lender);
    //     vm.stopPrank();
    ...
    //     assertEq(balance2 - balance1, rsa.totalOwed());
    // }
    ...
    //     assertEq(_rsa.totalOwed(), balance1 - balance2); // supposed to give 0 tokens
...
//     //     revenueToken.mint(address(rsa), revenue);
...
//     //     emit FeeClaimer.ClaimRevenue(revenueContract, address(revenueToken), revenue, revenueForOwner);
//     //     rsa.claimRevenue(revenueContract, address(revenueToken), claimData);
...
//     //     uint256 spigotBalance0 = revenueToken.balanceOf(address(rsa));

//     //     emit FeeClaimer.log_named_uint("balances", rsaBalance0);
//     //     emit FeeClaimer.log_named_uint("balances", spigotBalance0);

//     //     assertEq(rsaBalance0, 0);
//     //     assertEq(spigotBalance0, revenue);
//     //     // ensure our revenue logic matches with Spigot storage data
//     //     assertEq(rsa.getOwnerTokens(address(revenueToken)), claimableRev, "Spigot revenue doesnt match expected value");
...
//     //     uint256 spigotBalance1 = revenueToken.balanceOf(address(rsa));
//     //     assertEq(rsaBalance1, claimableRev);
//     //     assertEq(spigotBalance1, revenue - claimableRev);
//     //     assertEq(rsa.getOwnerTokens(address(revenueToken)), 0);
//     // }
...
//     //     address _revContract = makeAddr(0xdebf);
//     //     (uint8 split, , bytes4 _transferFunc) = rsa.getSetting(_revContract);
//     //      // ensure contract uninitialized and acutally setting new split
//     //     assertEq(split, 0);
//     //     assertEq(_transferFunc, bytes4(0));

//     //     vm.startPrank(lender);
//     //     vm.expectRevert(RevenueShareAgreement.NotLender.selector);
//     //     rsa.addFee(_revContract, claimPushPaymentFunc, transferOwnerFunc);
//     //     vm.stopPrank();
//     // }
...
//     //     rsa.addFee(_revContract, claimPushPaymentFunc, transferOwnerFunc);
//     //     vm.stopPrank();
//     //     (uint8 split2, , bytes4 _transferFunc2) = rsa.getSetting(_revContract);
...
//     //     bytes4 operateFunc = bytes4(0x12345678);
//     //     assertEq(rsa.isWhitelisted(operateFunc), false);
...
//     //     rsa.updateWhitelist(operateFunc, true);
//     //     vm.stopPrank();
//     //     assertEq(rsa.isWhitelisted(operateFunc), true);
//     // }

//     // function test_setRevenueSplit_mustUseInitializedRevenueSplit() public {
//     //     uint8 badSplit = 10;
//     //     address _revContract = _addRevenueContract(rsa, address(rsa), address(revenueToken), badSplit, claimPushPaymentFunc, transferOwnerFunc);
//     //     (uint8 split, , bytes4 _transferFunc) = rsa.getSetting(_revContract);
...
//     //     address[3] memory claimers = [rando, borrower, lender];
//     //     uint8 badSplit = uint8(bound(_badRevenueSplit, 0, 100));
...
//         vm.expectRevert(RevenueShareAgreement.NotBorrower.selector);
//         vm.startPrank(rando);
...
//         assertEq(rsa.owner(), address(rsa));
//         vm.startPrank(borrower);
//         rsa.releaseSpigot(borrower);
//         assertEq(rsa.owner(), borrower);
...
//     ) internal {
//         creditToken.mint(_lender, _rsa.totalOwed());
//         // deal(address(creditToken), _lender, rsa.totalOwed());
...
//         (uint8 split, bytes4 claimFunc, ) = rsa.getSetting(_revenueContract);
//         _token.mint(address(rsa), _amount);
...
//         rsa.claimRevenue(_revenueContract, address(_token), claimData);
...
//     //     // TODO no new contract its in RSA already
//     //     spigot = new Spigot(); 
//     //     rsa.initialize(borrower, operator);

//     //     // deploy new revenue contract with settings
//     //     revenueContract = _addRevenueContract(rsa, borrower, _token, _split, _claimFunc, _newOwnerFunc);
...
//     // ) internal returns(address) {
//     //     // deploy new revenue contract with settings
//     //     address newFeeContract = address(new MockFeeGenerator(_owner, _token));

//     //     settings = FeeClaimer.ClaimContract(_split, _claimFunc, _newOwnerFunc);

//     //     vm.startPrank(_owner);
//     //     // add spigot for revenue contract
//     //     require(
//     //         _claimer.addFeeContract(newFeeContract, settings),
//     //         "Failed to add spigot"
//     //     );

//     //     // give spigot ownership to claim revenue
//     //     newFeeContract.call(
//     //         abi.encodeWithSelector(_newOwnerFunc, address(rsa))
//     //     );
...
//     //         address(rsa).balance :
//     //         MockToken(_token).balanceOf(address(rsa));
...
//     //     assertEq(
//     //         rsa.getOwnerTokens(_token),
//     //         ownerTokens,
//     //         'Invalid Owner amount for spigot revenue'
//     //     );

//     //     assertEq(
//     //         rsa.getOperatorTokens(_token),
//     //         operatorTokens,
