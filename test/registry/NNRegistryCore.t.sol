...
contract NNRegistryCoreTest is FunETHBaseTest {
    ...
    function setUp() public virtual override {
        super.setUp();
        funUSDC = new FunETH();
        funUSDC.initialize(address(USDC), address(aave), address(debtWETH), "funUSDC", "funUSDC");
        registry = new FunCityLandRegistry();
        registry.initialize(address(funETH), address(funUSDC), curator);
    ...
    function test_constructor_setsNNValuesProperly() public {
        ...
        assertEq(address(registry.funUSDC()), address(funUSDC));
    }
    ...
    function test_stake_userCanStakeMultipleTimesToSameProperty() public {
        _listProperty();
        uint256 stakeID = _stakeProperty(address(vm.addr(3939)), address(funUSDC), lotID, 1_001 gwei);

        assertEq(registry.stakeCount(lotID), 1);
        assertEq(registry.includesStake(lotID, stakeID), true);

        uint256 newStakeID = _stakeProperty(address(vm.addr(3939)), address(funUSDC), lotID, 1_001 gwei);
    ...
    function test_stake_userCanStakeMultipleTimesToDifferentProperty() public {
        ...
        uint256 newStakeID = _stakeProperty(address(vm.addr(3939)), address(funUSDC), lot2, 1_001 gwei);
    ...
    // increases funUSDC supply
    // increases funUSDC balance of registry
